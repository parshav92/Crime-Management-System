...
class ShowDetailsWindow(tk.Toplevel):
    ...
    def setup_ui(self):
        ...
        if self.table_name == "criminal":
            ...
            self.tree.heading(5, text="Verdict")
            self.fetch_data("SELECT id, criminal_id, judge_name, hearing_date, verdict FROM court_record")
...
class CourtRecordsManagement(tk.Toplevel):
    ...
    def add_court_record(self):
        ...
        if not (criminal_id.isdigit() and judge_name and hearing_date and verdict):
            messagebox.showerror("Error", "Invalid input.")
            return
        ...
        try:
            cursor.execute('''
            INSERT INTO court_record (criminal_id, judge_name, hearing_date, verdict)
            VALUES (?, ?, ?, ?)
            ''', (criminal_id, judge_name, hearing_date, verdict))
            conn.commit()
